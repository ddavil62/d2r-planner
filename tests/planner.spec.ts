...
test('equips Obedience with its catalog identity, full options and calculated stats', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('combat-dps')).toHaveText('28')
  await expect(page.getByTestId('combat-final-hit')).toHaveText('12,809')
  await expect(page.getByTestId('combat-calculator')).toContainText('14프레임 · 초당 1.79회 · 명중 5%')
  await expect(page.getByTestId('combat-calculator')).toContainText('강타 40%')
  await expect(page.getByTestId('combat-calculator')).toContainText('화염 25% (-25)')
  await page.getByLabel('적 프리셋').selectOption('normal')
  await expect(page.getByTestId('combat-dps')).toHaveText('66')
  ...
  await combatCandidate.getByRole('button', { name: '비교' }).click()
  await expect(page.getByRole('heading', { name: '현재 장비 대비 전투 비교' })).toBeVisible()
  await expect(page.locator('.candidate-combat-stats')).toContainText('예상 DPS')
  ...
  await expect(page.getByTestId('combat-average-hit')).toHaveText('618')
  await expect(page.getByLabel('플레이어 수')).toHaveValue('8')
})
